...
static RigTForm g_skyRbt(Cvec3(0.0, 0.25, 4.0));   // -----------change
...
static RigTForm g_temp_frame = g_skyRbt; 											//--------------change----------------

// Use g_frame to store the framw which is using
// So when drawing the frame can be changed
static RigTForm g_frame = g_temp_frame; 											 //--------------change----------------

// add a cube 2 and its transformation
static RigTForm g_objectRbt[2] = {RigTForm(Cvec3(1,0,0)), RigTForm(Cvec3(-1,0,0))}; 	//--------------change----------------
...
static void drawStuff() {
  ...
  const RigTForm eyeRbt = g_frame;      									                                       // -----------------change
  ...
  // ===========
  //
  const RigTForm groundRbt = RigTForm();  // identity  // -----------------change
  Matrix4 MVM = rigTFormToMatrix(invEyeRbt * groundRbt); // -----------------change
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseLClickButton && !g_mouseRClickButton) { // left button down?
    m = RigTForm(Quat::makeXRotation(-dy) * Quat::makeYRotation(dx));// -----------------change
  }
  else if (g_mouseRClickButton && !g_mouseLClickButton) { // right button down?
    m = RigTForm(Cvec3(dx, dy, 0) * 0.01);// -----------------change
  }
  else if (g_mouseMClickButton || (g_mouseLClickButton && g_mouseRClickButton)) {  // middle or (left and right) button down?
    m = RigTForm(Cvec3(0, 0, -dy) * 0.01);// -----------------change
  }
  ...
  if (g_mouseClickDown) 
  {
    //g_objectRbt[g_object_number%3] *= m; // Simply right-multiply is WRONG
       ...
       if(frame_number == 0) // frame is sky                                                                            
       {
                ...
                {
                            RigTForm mix = g_objectRbt[object_number-1];
                            RigTForm A = transFact(mix) * linFact(g_frame); 
                            g_objectRbt[object_number-1] = A * m * inv(A) * mix;
                }
                g_frame = g_temp_frame;                                                                                    
       }
       else // frame is a cube                                                                                                    
       {
                if(object_number == 0) // object is sky
                {
                           cout<<"sky camera cannot be modified when current frame is a cube frame"<<endl;
                }
                else // object is a cube too                                                                                   
                {
                          RigTForm mix = g_objectRbt[object_number-1];
                          RigTForm A = transFact(mix) * linFact(g_objectRbt[frame_number-1]);
                          g_objectRbt[object_number-1] = A * m * inv(A) * mix;
                }
                g_frame = g_objectRbt[frame_number-1];                                                       
       }
    ...
    case 'v':
    	...
    	switch(g_frame_number%3)
    	{
    		...
    		case 1: g_frame = g_objectRbt[0];
    			 cout<<"Current frame is frame of cube 1."<<endl;		
    			 break;
    		case 2: g_frame = g_objectRbt[1];
    			 cout<<"Current frame is frame of cube 2."<<endl;		
    			 break;
    	}
    break;
    case 'o':
    	...
    	{
    		case 0: cout<<"Current object is sky camera."<<endl;		
    			 break;
    		case 1: cout<<"Current object is cube 1."<<endl;			
    			 break;
    		case 2: cout<<"Current object is cube 2."<<endl;			
    			 break;
    	}
    break;
    case 'm':

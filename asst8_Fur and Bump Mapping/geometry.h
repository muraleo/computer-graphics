...
public:
	...
	// new upload
	void new_upload(Mesh& mesh, bool isSmoothShading)//------change
	{
		if(!isSmoothShading)
		{
			VertexPN v;
			vector<VertexPN> vertices;
			int numVertices = 6 * mesh.getNumFaces();
			for(int i = 0; i<mesh.getNumFaces(); i++)
			{
				//cout<<"Face number is "<<mesh.getNumFaces()<<endl; //test
				for(int j=0; j<mesh.getFace(i).getNumVertices()-1;j++)
				{
					v = VertexPN(mesh.getFace(i).getVertex(j%4).getPosition(), mesh.getFace(i).getNormal());
					vertices.push_back(v);

					v = VertexPN(mesh.getFace(i).getVertex((j+1)%4).getPosition(), mesh.getFace(i).getNormal());
					vertices.push_back(v);

					v = VertexPN(mesh.getFace(i).getVertex((j+2)%4).getPosition(), mesh.getFace(i).getNormal());
					vertices.push_back(v);

					j++;
				}
			}
			upload(&vertices[0], numVertices);
		}
		else
		{
			//cout<<"face number is "<<mesh.getNumVertices()<<endl; //test
			mesh.updateNormals();
			VertexPN v;
			vector<VertexPN> vertices;
			//int numVertices = 6 * mesh.getNumFaces();
			for(int i = 0; i<mesh.getNumFaces(); i++)
			{
				for(int j=0; j<mesh.getFace(i).getNumVertices()-1;j++)
				{
					v = VertexPN(mesh.getFace(i).getVertex(j%4).getPosition(), mesh.getFace(i).getVertex(j%4).getNormal());
					vertices.push_back(v);

					v = VertexPN(mesh.getFace(i).getVertex((j+1)%4).getPosition(), mesh.getFace(i).getVertex((j+1)%4).getNormal());
					vertices.push_back(v);

					v = VertexPN(mesh.getFace(i).getVertex((j+2)%4).getPosition(), mesh.getFace(i).getVertex((j+2)%4).getNormal());
					vertices.push_back(v);

					j++;
				}
			}
			upload(&vertices[0], vertices.size());//------change
		}
	}
};

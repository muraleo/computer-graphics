...
public:
	...
	void updateNormals()//------change
	{
		//cout<<"test 3"<<endl;test
		//cout<<"face number is "<<(*this).getNumVertices()<<endl; //test
		for (int i = 0; i < (*this).getNumVertices(); i++)  // zero out all of the vertex normals
		{
			Mesh::Vertex v = (*this).getVertex(i);
			v.setNormal(Cvec3(0,0,0));
			//cout<<i<<endl;
		}
		for (int i = 0; i < (*this).getNumFaces(); i++)  // get accumulate normals for every vertex
		{
			//cout<<"Face's vertex number "<<(*this).getFace(i).getNumVertices()<<endl;
			for(int j = 0; j<(*this).getFace(i).getNumVertices();j++)
			{
				Cvec3 normal = (*this).getFace(i).getVertex(j).getNormal() + (*this).getFace(i).getNormal();
				(*this).getFace(i).getVertex(j).setNormal(normal);
			}
		}
		for (int i = 0; i < (*this).getNumVertices(); i++) // set average normals to each vertex
		{
			Mesh::Vertex v = (*this).getVertex(i);
			Cvec3 normal = v.getNormal();
			v.setNormal(normalize(normal));
		}
	}
};

...
template<typename T, int n> //------change
...
template<typename T, int n>     //------change
inline Cvec<T, n> lerp(const Cvec<T, n> v0, const Cvec<T, n> v1, const T alpha)
{
	//cout<<"test17"<<endl;
	return v0 * (1.0 - alpha) + v1*alpha;	// number can be only multiple ob the right side!
}

//------change
template<typename T, int n>
inline Cvec<T, n> interpolateCatmullRom(const Cvec<T, n> v0, const Cvec<T, n> v1, const Cvec<T, n> v2, const Cvec<T, n> v3, const T alpha)
{
	Cvec<T, n> d = ( v2 - v0 ) * (1.0/6.0) + v1; 
	Cvec<T, n> e = ( v3 - v1 ) * (1.0/6.0) * (-1) + v2;
	Cvec<T, n> f = v1 * ( 1 - alpha ) + d * alpha;
	Cvec<T, n> g = d * ( 1 - alpha ) + e * alpha;
	Cvec<T, n> h = e * ( 1 - alpha ) + v2 * alpha;
	Cvec<T, n> m = f * ( 1 - alpha ) + g * alpha;
	Cvec<T, n> n1 = g * ( 1 - alpha ) + h * alpha;
	Cvec<T, n> c = m * ( 1 - alpha ) + n1 * alpha;
	return c;
}

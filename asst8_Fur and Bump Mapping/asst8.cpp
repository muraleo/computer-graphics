...
static list<std::vector<RigTForm> > key_frame; 								
static list<std::vector<RigTForm> >::iterator current_key_frame = key_frame.begin(); 				// remember to initiallize the interator 
static int key_frame_index = 0; // current number od the current key frame
...
static void initBunnyMeshes() //------change
{
	g_bunnyMesh.load("bunny.mesh");
	g_bunnyGeometry.reset(new SimpleGeometryPN());
	// initial bunny with smooth shading to get the correct normal of every vertex
	g_bunnyGeometry->new_upload(g_bunnyMesh, true);
...
static void updateShellGeometry(); //------change
static void drawStuff(bool picking)
{
	if(g_shellNeedsUpdate == true)//------change
	{
		updateShellGeometry();
	}
...
static bool bubbling_interpolateAndDisplay(float t)
{
	mesh_temp = mesh;
...
static void bubbling_animation()
{
...
static void bubbling_animation_faster()	
{
...
static void bubbling_animatino_slower()
{
		bubbling_animate_speed = bubbling_animate_speed *2;
		cout << "Bubbling animate speed is " << bubbling_animate_speed << endl;
}
...
RigTForm bunny_rtf, inv_bunny_rtf;

static void updateShellGeometry() //------change
{    
	// TASK 1 and 3 TODO: finish this function as part of Task 1 and Task 2
	inv_bunny_rtf = inv(getPathAccumRbt(g_world, g_bunnyNode)); // put getpath function here to avoid call it for everytimes
    	vector<VertexPNX> ver;
    	Cvec2 text;
    	Cvec3 p, tip, n, d;
    	Cvec4 temp;
    	int vertex_index;
      		...
      		for (int j = 0; j < g_bunnyMesh.getNumFaces(); j++) 
       		{
            	for (int l = 0; l < 3; l++) 
            	{
    				p = g_bunnyMesh.getFace(j).getVertex(l).getPosition();
    				vertex_index = g_bunnyMesh.getFace(j).getVertex(l).getIndex();
    				temp = inv_bunny_rtf * Cvec4(g_tipPos[vertex_index], 1);
    				tip = Cvec3(temp(0), temp(1), temp(2));
    				n = g_bunnyMesh.getFace(j).getVertex(l).getNormal()*(g_furHeight / g_numShells); // n = (g_furHeight)/m * N
    				if(l==0)
    					text = Cvec2(0, 0);
    				else if(l==1)
    					text = Cvec2(g_hairyness, 0);
    				else
    					text = Cvec2(0, g_hairyness);
    				//task1 and task2
    				//p = p +n*i;
    				//n= (tip-p)/g_numShells;

    				//task3
    				// Because t = p + m*n + d+2d+3d+...+(m-1)d = p+ m*n + ((m-1)*m/2)*d
    				// so d = 2(t-p-m*n)/((m-1)*m)
    				d = ((tip-p-n*g_numShells)*2.0)/ ((g_numShells-1) * g_numShells);
    				p = p + (n*i)+d*((i-1)*i/2.0);
    				n = n+d*(i-1); // p_i-p_i-1 is normal = n +(i-1)*d
    				ver.push_back(VertexPNX(p, n, text));
            	}
        	}
        	g_bunnyShellGeometries[i]->upload(&ver[0], ver.size());
    	}
...
static Cvec3 get_world_coor(Cvec3 t) // get the world coordinate of the object
{
	//RigTForm bunny_rtf = getPathAccumRbt(g_world, g_bunnyNode);
	Cvec4 temp = bunny_rtf * Cvec4(t, 1);
	Cvec3 gwc = Cvec3(temp(0), temp(1), temp(2));
	return gwc;
}//------change

static void hairsSimulationCallback(int dontCare) //------change
{

	// TASK 2 TODO: write dynamics simulation code here as part of TASK2
	Cvec3 p, N_cap, s, sf, total_force;
	bunny_rtf = getPathAccumRbt(g_world, g_bunnyNode); // put getpath function here to avoid call it for everytimes
	for(int i = 0; i<g_numStepsPerFrame; i++)
	{
		for(int j = 0; j<g_bunnyMesh.getNumVertices(); j++)
		{
			//convert object coordinate to world coordinate
			p = get_world_coor(g_bunnyMesh.getVertex(j).getPosition());
			N_cap = get_world_coor(g_bunnyMesh.getVertex(j).getNormal());
			s = p+N_cap*g_furHeight; // s = p+N*g_furHeight

			// compute the total force
			sf = (s - g_tipPos[j])*g_stiffness; // springforce  = (s-t)*stiffness
            total_force = sf+ g_gravity;

            //update the tip position
			g_tipPos[j] = g_tipPos[j] + g_tipVelocity[j] * g_timeStep;//t=t+Tv
            		
            //constrain t = p+g_furHeight*(t-p)/|t-p|
            g_tipPos[j] = p + (g_tipPos[j]-p) * (g_furHeight/ norm(g_tipPos[j]-p));
            
            //update velocity v = (v+Tf) * damping
            g_tipVelocity[j] = (g_tipVelocity[j] + total_force * g_timeStep) * g_damping;
		}
	}
...
static void initSimulation() //------change
{
	g_tipPos.resize(g_bunnyMesh.getNumVertices(), Cvec3(0));
	g_tipVelocity = g_tipPos; // initialize g_tipVelocity with 0

	// TASK 1 TODO: initialize g_tipPos to "at-rest" hair tips in world coordinates
	bunny_rtf = getPathAccumRbt(g_world, g_bunnyNode); // put getpath function here to avoid call it for everytimes
	Cvec3 p, N_cap; // p is the root, n_cap is the N
	for(int i = 0; i<g_bunnyMesh.getNumVertices();i++)
	{
		p = g_bunnyMesh.getVertex(i).getPosition();
		N_cap = g_bunnyMesh.getVertex(i).getNormal();
		g_tipPos[i] = get_world_coor(p+N_cap*g_furHeight); // s = p+n*g_furHeight
	}
...
static void initMaterials()
{
  ...
  // bunny material
	...
	shared_ptr<Texture> shellTexture(new ImageTexture("shell.ppm", false)); // common shell texture
...
static void initScene() 
{
	...
	g_currentPickedRbtNode=g_frameNode = g_skyNode;
	...
	g_bunnyNode.reset(new SgRbtNode(RigTForm(Cvec3(0,0,0.5))));
...
static void reset() 		
{
	...
	if(flag == 0)
	{
		...
		key_frame_index = 0; // current number od the current key frame
		g_msBetweenKeyFrames = 2000; // 2 seconds between keyframes
		g_animateFramesPerSecond = 60; // frames to render per second during animation playback
